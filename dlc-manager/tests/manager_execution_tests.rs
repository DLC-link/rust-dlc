...
fn manager_execution_test(test_params: TestParams, path: TestPath) {
    ...
    let alice_simple_wallet =
        simple_wallet::SimpleWallet::new(electrs.clone(), alice_store.clone(), Network::Regtest);
    let bob_simple_wallet =
        simple_wallet::SimpleWallet::new(electrs.clone(), bob_store.clone(), Network::Regtest);
