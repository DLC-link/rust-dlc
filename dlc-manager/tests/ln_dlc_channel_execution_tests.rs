...
use bitcoin::{hashes::Hash, Address, Amount, Network, Script, Transaction, TxIn, TxOut, Witness};
...
fn ln_dlc_test(test_path: TestPath) {
    ...
    let bob_fund_address = bob_node.wallet.get_new_address().unwrap();
    ...
    alice_node.wallet.refresh().unwrap();
    bob_node.wallet.refresh().unwrap();
